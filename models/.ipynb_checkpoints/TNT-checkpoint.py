...
def normalize_row(target_vector):
    ...
    target_hat = target_vector / torch.norm(target_vector, p=2, dim=-1, keepdim=True)
...
def kernels_cluster(weights_f, channel=True):
    """
...
#         permute_weights = weights_f.reshape(32, -1)
...
#             permute_weights = weights_f.reshape(o * i, h_ks * w_ks)  # (o, i, h, w) ==> (o * i, h * w)
    ...
    ternary_weights, cosine_similarity = TNT_convert(permute_weights)  # output the best ternary tensor and its cosine similarity with floating type
    weights_t = scaling(permute_weights, ternary_weights)
